...
export default function Editor() {
  ...
  // --- Cloud persistence (extracted to useCloudFlow: load/save/versions,
  // optimistic-concurrency conflict handling, autosave + dirty tracking, and
  // the unsaved-changes navigation guards). See the hook for the #30 note on
  // why its save handlers are deliberately unmemoized.
  const {
    handleCloudSave,
    handleSaveVersion,
  ...
  useClipboard({
    nodes, edges, selectedIds, setSelectedIds, path,
    isContextReadOnly, isAdmin, isSharedView, setGraph,
  });
  ...
  // so the (mount-once) key listener never captures a stale closure. Assigned in
  // an every-render effect (not during render) — the listener only reads it at
  // event time, after render.
